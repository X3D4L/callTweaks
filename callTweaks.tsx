...
function autoCloseEndedStream() {
    const s = Vencord.Settings.plugins.callTweaks;
    if (!s?.autoCloseEndedStream) return;
    
    const headers = document.querySelectorAll('[class*="header__29209"]');
    
    for (const h of headers) {
        if (h.textContent?.includes("This stream has ended")) {
            const closeBtn = h.parentElement?.querySelector("button");
...
function injectCSS() {
    ...
    style.textContent = `
        ...
        ${s?.alwaysShowTopBar && !s?.normalTopBarWhenStreaming ? `[class*="topControls"] { opacity: 1 !important; pointer-events: auto !important; transform: none !important; }` : ""}
        ${s?.alwaysShowBottomBar && !s?.normalBottomBarWhenStreaming ? `[class*="bottomControls"] { opacity: 1 !important; pointer-events: auto !important; transform: none !important; }` : ""}
        ${s?.hideShadow ? `[class*="gradientTop"], [class*="gradientBottom"], [class*="overlayContainer"] { display: none !important; }` : ""}
        ${s?.hideStreamInfo ? `[class*="streamQualityIndicator"], [class*="liveIndicator"] { display: none !important; }` : ""}
        ${s?.hideAKA ? `[class*="akaBadge"], [class*="nicknames"], [class*="divider"] { display: none !important; }` : ""}
        ${s?.hideStreamPreview ? `[class*="streamPreview__"] img { display: none !important; } [class*="streamPreviewOpacity__"] { opacity: 0 !important; pointer-events: none !important; } [class*="addCta__"] { display: none !important; }` : ""}
